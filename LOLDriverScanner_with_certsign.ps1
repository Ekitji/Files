...
foreach ($driver in $drivers) {
    ...
    try {# Calculate the SHA256, SHA1, and MD5 hashes of the driver file
        ...
        # Certificate checks
        $CertIssuer = (Get-AuthenticodeSignature -FilePath $driver.FullName).SignerCertificate.Issuer -split ',' | Select-String "O=","CN"
        $CertSubject = (Get-AuthenticodeSignature -FilePath $driver.FullName).SignerCertificate.Subject -split ',' | Select-String "O=","CN"
        $CertStatus = (Get-AuthenticodeSignature -FilePath $driver.FullName).Status
        $CertStatusMsg = (Get-AuthenticodeSignature -FilePath $driver.FullName).StatusMessage
